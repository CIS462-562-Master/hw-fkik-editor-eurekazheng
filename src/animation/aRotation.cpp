...
void quat::FromRotation(const mat3& rot)
{
    ...
    if (mQ[VW] != 0) {
        ...
        mQ[VX] = sqrt((rot[0][0] + 1 - 2 * mQ[VW] * mQ[VW]) / 2.0);
        mQ[VY] = sqrt((rot[1][1] + 1 - 2 * mQ[VW] * mQ[VW]) / 2.0);
        mQ[VZ] = sqrt((rot[2][2] + 1 - 2 * mQ[VW] * mQ[VW]) / 2.0);
    }
...
quat quat::Slerp(const quat& q0, const quat& q1, double u)
{
    ...
    q = sin((1 - u) * angle) / sin(angle) * q0 + 
        sin(angle * u) / sin(angle) * q1;
...
{
    quat q;
    //TODO: student implementation of SDouble goes here
