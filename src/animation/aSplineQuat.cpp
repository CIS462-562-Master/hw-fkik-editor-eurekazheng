...
void ASplineQuat::computeControlPoints(quat& startQuat, quat& endQuat)
{
	...
	for (int segment = 0; segment < numKeys - 1; segment++)
	{
		...
		quat q10 = quat::SDouble(q_1, q0);
		quat q11 = quat::SBisect(q10, q1);
		quat q00 = quat::SDouble(q2, q1);
		quat q01 = quat::SBisect(q0, q00);

		b1 = quat::Slerp(q0, q11, 0.33f);
		b2 = quat::Slerp(q1, q01, 0.33f);
...
quat ASplineQuat::getLinearValue(double t)
{
	...
	double u = (t - mKeys[segment].first) / 
			   (mKeys[segment + 1].first - mKeys[segment].first);
